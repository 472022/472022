<!--
═══════════════════════════════════════════════════════════════════════════════
  PRATHAMESH HALALE — GitHub Profile README   ·   account: @472022
  ─────────────────────────────────────────────────────────────────────────────
  Palette   BG #1A1B26 · ACCENT #FF9E64 · ACCENT-2 #BB9AF7 · TEXT #C0CAF5
  Layout    Centered <div> and <table> only. GitHub strips CSS, so every bit of
            styling is done with HTML attributes and image query params.
  Rhythm    No `---` rules. GitHub already renders an <h2> underline; adding
            horizontal rules on top of that flattens the hierarchy.

  ⚠ ONE SWAP POINT before this renders fully — see PROFILE-GUIDE.md § Setup.
    Search this file for  [SWAP-1]  (stats)  and  [SWAP-2]  (trophies).
═══════════════════════════════════════════════════════════════════════════════
-->

<!-- ─────────────────────────────  HEADER BANNER  ───────────────────────────── -->

<div align="center">

<img
  src="https://media1.tenor.com/m/-W03P8wXXgMAAAAd/anime-scenery.gif"
  alt="Anime Scenery Header Banner"
  width="100%"
/>

<br />

<h1 align="center">✧ &nbsp; Prathamesh Halale &nbsp; ✧</h1>
<h3 align="center"><i>Software Engineer · Full Stack · Mobile · AI · Cloud</i></h3>

<!-- ────────────────────────────  TYPING ANIMATION  ─────────────────────────── -->

<a href="https://github.com/472022">
  <img
    src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1200&color=FF9E64&center=true&vCenter=true&width=760&height=60&lines=Building+scalable+SaaS+%26+AI+systems;Kotlin+Android%2C+shipped+to+production;LLM+orchestration+at+multi-tenant+scale;Computer+vision+with+YOLOv8+%2B+PyTorch;Problem+Solver+%7C+Open+Source+Enthusiast"
    alt="Building scalable SaaS and AI systems"
  />
</a>

<br />


<!-- ───────────────────────────────  META BADGES  ──────────────────────────── -->

<a href="https://github.com/472022">
  <img src="https://komarev.com/ghpvc/?username=472022&color=FF9E64&style=flat-square&label=PROFILE+VIEWS&label_color=1A1B26" alt="Profile views" />
</a>
<a href="https://github.com/472022?tab=followers">
  <img src="https://img.shields.io/github/followers/472022?style=flat-square&color=BB9AF7&labelColor=1A1B26&logo=github&logoColor=BB9AF7&label=FOLLOWERS" alt="GitHub followers" />
</a>
<a href="https://github.com/472022?tab=repositories">
  <img src="https://img.shields.io/badge/PUBLIC_REPOS-77-FF9E64?style=flat-square&labelColor=1A1B26&logo=github&logoColor=FF9E64" alt="Public repositories" />
</a>
<a href="mailto:prathameshhalale@gmail.com">
  <img src="https://img.shields.io/badge/STATUS-OPEN_TO_OPPORTUNITIES-BB9AF7?style=flat-square&labelColor=1A1B26" alt="Open to opportunities" />
</a>

</div>

<br />

<!-- ═══════════════════════════════  ABOUT  ══════════════════════════════════ -->

## 🌸 &nbsp;私について | About Me

<table>
<tr>
<td width="60%" valign="top">

> I'm a software engineer who ships across the whole stack — from **Kotlin Android apps** with biometric auth and encrypted storage, to **multi-tenant AI platforms** that route traffic across half a dozen LLM providers, to **computer-vision backends** running YOLOv8 over live video.<br><br>Most of what I build is full-stack by necessity: a Django or FastAPI service, a React front end, a Postgres/Redis layer, all wired together with Docker. I care about systems that stay up, degrade gracefully, and are honest about their own confidence.

<br/>

**🧭 &nbsp;What I do**
- 🏗️ Design and ship **full-stack products** end to end
- 📱 Build **native Android** apps in Kotlin
- 🤖 Engineer **LLM routing, evaluation & agent** systems
- 👁️ Train and deploy **computer-vision** pipelines

</td>
<td width="40%" valign="top" align="center">

<img src="https://i.pinimg.com/originals/a4/f3/f1/a4f3f15053cf200e6203cf3d75bf6936.gif" alt="lo-fi coding anime gif" width="100%" />

<br/>

**⚙️ &nbsp;How I work**
- 🧩 Break problems into shippable slices
- 📊 Metrics over vibes
- ♻️ Prefer boring, maintainable tech

</td>
</tr>
</table>

<!-- ═══════════════════════════  CURRENT FOCUS  ═════════════════════════════ -->

## 🎯 &nbsp;現在のフォーカス | Current Focus

<table>
<tr>
<td width="33%" valign="top" align="center">

### 🔭 &nbsp;Building

**Manthan.ai Orchestrator**

Intent-aware LLM routing with
quality and hallucination scoring

</td>
<td width="33%" valign="top" align="center">

### 📱 &nbsp;Shipping

**Tradora**

Kotlin Android trading client
with biometric auth

</td>
<td width="33%" valign="top" align="center">

### 🌱 &nbsp;Learning

**Distributed Systems**

Evaluation harnesses, model
observability, Kubernetes

</td>
</tr>
</table>

<!-- ═══════════════════════════════  TECH STACK  ════════════════════════════ -->

## ⚔️ &nbsp;技術スタック | Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=kotlin,java,python,ts,js,php,bash&theme=dark" alt="Kotlin, Java, Python, TypeScript, JavaScript, PHP, Bash" />

**Mobile**

<img src="https://skillicons.dev/icons?i=kotlin,androidstudio,gradle,materialui&theme=dark" alt="Kotlin, Android Studio, Gradle, Material Design" />

**Backend & Frameworks**

<img src="https://skillicons.dev/icons?i=django,fastapi,flask,nodejs,express,php&theme=dark" alt="Django, FastAPI, Flask, Node.js, Express, PHP" />

**AI · ML · LLM**

<img src="https://skillicons.dev/icons?i=pytorch,opencv,sklearn,python&theme=dark" alt="PyTorch, OpenCV, scikit-learn, Python" />

<img src="https://img.shields.io/badge/YOLOv8-FF9E64?style=flat-square&logo=ultralytics&logoColor=1A1B26" alt="YOLOv8" />
<img src="https://img.shields.io/badge/Anthropic-BB9AF7?style=flat-square&logo=anthropic&logoColor=white" alt="Anthropic" />
<!-- simple-icons no longer ships an `openai` logo, so this badge is text-only by design. -->
<img src="https://img.shields.io/badge/OpenAI_API-161B22?style=flat-square&labelColor=161B22&color=FF9E64" alt="OpenAI API" />
<img src="https://img.shields.io/badge/LangChain-FF9E64?style=flat-square&logo=langchain&logoColor=1A1B26" alt="LangChain" />
<img src="https://img.shields.io/badge/LiveKit-BB9AF7?style=flat-square&logo=livekit&logoColor=white" alt="LiveKit" />
<img src="https://img.shields.io/badge/MediaPipe-161B22?style=flat-square&logo=google&logoColor=FF9E64" alt="MediaPipe" />

**Frontend**

<img src="https://skillicons.dev/icons?i=react,vite,tailwind,html,css,bootstrap&theme=dark" alt="React, Vite, Tailwind CSS, HTML5, CSS3, Bootstrap" />

**Data & Infrastructure**

<img src="https://skillicons.dev/icons?i=postgres,mysql,redis,sqlite,docker,nginx&theme=dark" alt="PostgreSQL, MySQL, Redis, SQLite, Docker, Nginx" />

**Cloud & Tooling**

<img src="https://skillicons.dev/icons?i=aws,githubactions,git,github,linux,postman,figma&theme=dark" alt="AWS, GitHub Actions, Git, GitHub, Linux, Postman, Figma" />

</div>

<details>
<summary><b>📖 &nbsp;The full stack, in detail</b></summary>

<br />

| Layer | Technologies |
|---|---|
| **Android** | Kotlin · Retrofit · OkHttp · Glide · Material 3 · Navigation Component · ViewModel / LiveData · AndroidX Biometric · Security-Crypto · Shimmer · ViewPager2 · Espresso / JUnit |
| **AI & Computer Vision** | PyTorch · torchvision · Ultralytics YOLOv8 · OpenCV · MediaPipe · face_recognition · scikit-learn · NumPy · Pillow · joblib |
| **LLM & Agents** | Anthropic · OpenAI-compatible APIs · NVIDIA NIM · Groq · LiveKit Agents · Pipecat · Deepgram · Cartesia · Silero · Tavily · mem0 · LangChain |
| **Backend** | Django 5 · Django REST Framework · django-ninja · FastAPI · Flask · Django Channels / Daphne · Celery · SimpleJWT · Node.js · Express · PHP |
| **Data** | PostgreSQL 15 · MySQL · Redis 7 · SQLite · pandas · openpyxl |
| **Infra & Observability** | Docker · Docker Compose · Nginx · Prometheus · Flower · GitHub Actions · Render |
| **Frontend** | React · Vite · TypeScript · Tailwind CSS v4 · JavaScript · HTML5 · CSS3 · Bootstrap |
| **Automation & QA** | Playwright · Espresso · Postman |

</details>

<!-- ═════════════════════════  FEATURED PROJECTS  ═══════════════════════════ -->

## ⛩️ &nbsp;注目のプロジェクト | Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🧠 &nbsp;Manthan.ai — LLM Orchestrator

A multi-tenant SaaS exposing a single **OpenAI-compatible** `/v1/chat/completions` endpoint that
routes each request to the best model based on classified intent, complexity and live provider
health — then scores the response for quality and hallucination risk. A background learning loop
re-ranks providers from real traffic and promotes better "challenger" models via A/B testing.

`Django + django-ninja` `FastAPI` `React + Vite + TS`
`Celery` `Redis` `PostgreSQL` `Docker` `Prometheus`

<img src="https://img.shields.io/badge/STATUS-IN_DEV-BB9AF7?style=flat-square&labelColor=1A1B26" alt="In development" />
<img src="https://img.shields.io/badge/repo-private-6E7681?style=flat-square&labelColor=1A1B26" alt="Private repo" />

</td>
<td width="50%" valign="top">

### 🚁 &nbsp;DroneEye — Inspection Dashboard

A real-time operator dashboard for **drone-based transmission line inspection**. Streams live
telemetry from ESP32 hardware, with a built-in simulator so the whole stack can be exercised
without a drone attached. Deployable to Render with automatic port configuration.

`Python` `Flask` `ESP32` `Realtime telemetry`

<img src="https://img.shields.io/badge/STATUS-ACTIVE-FF9E64?style=flat-square&labelColor=1A1B26" alt="Active" />

<a href="https://github.com/472022/DroneEye">
  <img src="https://img.shields.io/badge/GitHub-View_Repo-161B22?style=flat-square&logo=github&logoColor=FF9E64" alt="View DroneEye repo" />
</a>

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🎙️ &nbsp;Dora — Realtime Voice Agent

A Jarvis-style conversational assistant with **web search, vision through the camera, weather,
and email**, driven by a low-latency voice pipeline over **LiveKit**. Built on Pipecat with local
smart-turn detection, Deepgram/Cartesia speech and persistent recall through `mem0`.

`Python 3.12` `LiveKit` `Pipecat` `Groq`
`Deepgram` `Cartesia` `Tavily` `mem0`

<img src="https://img.shields.io/badge/STATUS-DEPLOYED-FF9E64?style=flat-square&labelColor=1A1B26" alt="Deployed" />

<a href="https://github.com/472022/doraDeployed">
  <img src="https://img.shields.io/badge/GitHub-View_Repo-161B22?style=flat-square&logo=github&logoColor=FF9E64" alt="View Dora repo" />
</a>

</td>
<td width="50%" valign="top">

### 🏭 &nbsp;Process Plant AI

A marketing site plus an embedded **Lab Digitiser** product surface, backed by a small
Node/Express contact service with SMTP delivery. Hand-rolled HTML/CSS/JS — no bundler — with
compiled Tailwind v4 output and iframe-based product routing.

`TypeScript` `Express` `Nodemailer` `Tailwind v4`

<img src="https://img.shields.io/badge/STATUS-DEPLOYED-FF9E64?style=flat-square&labelColor=1A1B26" alt="Deployed" />

<a href="https://github.com/472022/ProcessPlantAi">
  <img src="https://img.shields.io/badge/GitHub-View_Repo-161B22?style=flat-square&logo=github&logoColor=FF9E64" alt="View Process Plant AI repo" />
</a>

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 📈 &nbsp;Tradora — Android Trading Client

A native Kotlin trading app targeting **Android 16 (SDK 36)**. Built on a ViewModel/LiveData
architecture with Retrofit + OkHttp networking, biometric authentication, and `Security-Crypto`
encrypted storage for credentials. Shimmer skeletons and swipe-to-refresh throughout.

`Kotlin` `Retrofit` `OkHttp` `Material 3`
`Biometric` `Security-Crypto` `Navigation` `Glide`

<img src="https://img.shields.io/badge/STATUS-ACTIVE-FF9E64?style=flat-square&labelColor=1A1B26" alt="Active" />
<img src="https://img.shields.io/badge/repo-publishing_soon-6E7681?style=flat-square&labelColor=1A1B26" alt="Publishing soon" />

</td>
<td width="50%" valign="top">

### 👁️ &nbsp;AI Surveillance System

Real-time video analytics: **YOLOv8** object detection and face recognition streamed to clients
over WebSockets via Django Channels/Daphne. JWT-secured REST API, fully containerised, with a
React front end and its own model documentation.

`Django 5` `Channels` `YOLOv8` `PyTorch`
`OpenCV` `MediaPipe` `SimpleJWT` `Docker`

<img src="https://img.shields.io/badge/status-prototype-BB9AF7?style=flat-square&labelColor=1A1B26" alt="Prototype" />
<img src="https://img.shields.io/badge/repo-private-6E7681?style=flat-square&labelColor=1A1B26" alt="Private repo" />

</td>
</tr>
</table>

<div align="center">

<a href="https://github.com/472022?tab=repositories">
  <img src="https://img.shields.io/badge/Browse_all_77_repositories-161B22?style=for-the-badge&logo=github&logoColor=FF9E64" alt="Browse all repositories" />
</a>

</div>

<!-- ═══════════════════════════  OPEN SOURCE  ═══════════════════════════════ -->

## 🌍 &nbsp;オープンソース | Open Source

<div align="center">

I build in the open where I can, and I'm actively working toward publishing more of the above.
If any of these problem spaces overlap with yours — **LLM routing, on-device ML, Android
security, or realtime voice** — I'd genuinely like to hear from you.

<br />

<a href="https://github.com/472022?tab=repositories">
  <img src="https://img.shields.io/badge/Open_to-COLLABORATION-FF9E64?style=for-the-badge&labelColor=1A1B26" alt="Open to collaboration" />
</a>
<a href="mailto:prathameshhalale@gmail.com">
  <img src="https://img.shields.io/badge/Open_to-CONTRIBUTIONS-BB9AF7?style=for-the-badge&labelColor=1A1B26" alt="Open to contributions" />
</a>

</div>

<!-- ══════════════════════════  GITHUB STATISTICS  ══════════════════════════ -->

## 📊 &nbsp;GitHubの統計 | GitHub Statistics

<!--
  [SWAP-1] ── github-readme-stats.vercel.app is currently 503 DEPLOYMENT_PAUSED.
  Deploy your own fork, then run from this directory:
    sed -i '' 's|github-readme-stats.vercel.app|YOUR-HOST.vercel.app|g' README.md
  Full walkthrough in PROFILE-GUIDE.md § Setup.
-->

<div align="center">

<img
  height="170"
  src="https://github-readme-stats.vercel.app/api?username=472022&show_icons=true&count_private=true&include_all_commits=true&hide_border=true&bg_color=1A1B26&title_color=FF9E64&icon_color=BB9AF7&text_color=C0CAF5&ring_color=FF9E64"
  alt="Prathamesh Halale's GitHub stats"
/>
<img
  height="170"
  src="https://github-readme-stats.vercel.app/api/top-langs/?username=472022&layout=compact&langs_count=8&hide_border=true&bg_color=1A1B26&title_color=FF9E64&text_color=C0CAF5"
  alt="Most used languages"
/>

<br /><br />

<img
  src="https://streak-stats.demolab.com?user=472022&hide_border=true&background=1A1B26&border=1A1B26&stroke=21262D&ring=FF9E64&fire=BB9AF7&currStreakLabel=FF9E64&sideLabels=C0CAF5&currStreakNum=C0CAF5&sideNums=C0CAF5&dates=6E7681"
  alt="GitHub streak"
/>

</div>

<!-- ═══════════════════════  CONTRIBUTION CALENDAR  ═════════════════════════ -->

<!--
  NOTE: github-readme-activity-graph was removed on purpose. It returns HTTP 200 but
  renders "Can't fetch any contribution" inside the SVG — verified broken for every
  username tested, including torvalds and gaearon, so it is a service-side outage,
  not a username problem. ghchart below is content-verified against real data.
-->

## 📅 &nbsp;貢献カレンダー | Contribution Calendar

<div align="center">

<img src="https://ghchart.rshah.org/FF9E64/472022" alt="Contribution calendar" width="100%" />

</div>

<!-- ═════════════════════════  CONTRIBUTION SNAKE  ══════════════════════════ -->

## 🐍 &nbsp;コントリビューションスネーク | Contribution Snake

<!--
  Generated by .github/workflows/snake.yml (Platane/snk) into the `output` branch.
  These URLs 404 until that workflow has run once — trigger it from the Actions tab.
-->

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)"  srcset="https://raw.githubusercontent.com/472022/472022/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/472022/472022/output/github-snake.svg" />
  <img alt="Snake eating my contribution graph" src="https://raw.githubusercontent.com/472022/472022/output/github-snake.svg" width="100%" />
</picture>

</div>

<!-- ═══════════════════════════════  TROPHIES  ══════════════════════════════ -->

## 🏆 &nbsp;実績 | Trophies & Achievements

<!--
  [SWAP-2] ── github-profile-trophy.vercel.app is currently 402 DEPLOYMENT_DISABLED.
  Deploy your own fork, then run:
    sed -i '' 's|github-profile-trophy.vercel.app|YOUR-TROPHY-HOST.vercel.app|g' README.md
-->

<div align="center">

<img
  src="https://github-profile-trophy.vercel.app/?username=472022&theme=radical&no-frame=true&no-bg=true&column=7&margin-w=8&margin-h=8"
  alt="GitHub trophies"
  width="100%"
/>

</div>

<!-- ═════════════════════  EXPERIENCE & CERTIFICATIONS  ═════════════════════ -->

## 💼 &nbsp;経験と資格 | Experience & Certifications

<table>
<tr>
<td width="50%" valign="top">

**💼 &nbsp;Experience**

**Full-Stack & AI Engineer** — *Independent*
Designing and shipping SaaS platforms, Android apps
and AI systems end to end.

**Web Application Developer**
Production PHP/MySQL business systems — catering,
billing, inventory, CRM and ERP tooling.

<sub>📄 Detailed CV available on request.</sub>

</td>
<td width="50%" valign="top">

**🎓 &nbsp;Certifications**

<sub>*This layout is ready for your certifications —
drop them in using this badge format:*</sub>

<img src="https://img.shields.io/badge/Certification_Name-Issuer-FF9E64?style=flat-square&labelColor=1A1B26" alt="Certification example" />
<img src="https://img.shields.io/badge/Certification_Name-Issuer-BB9AF7?style=flat-square&labelColor=1A1B26" alt="Certification example" />

<sub>See PROFILE-GUIDE.md § Certifications for
copy-paste templates.</sub>

</td>
</tr>
</table>

<!-- ═══════════════════════════════  CONNECT  ═══════════════════════════════ -->

## 🤝 &nbsp;連絡する | Connect With Me

<div align="center">

<a href="mailto:prathameshhalale@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
<a href="https://github.com/472022">
  <img src="https://img.shields.io/badge/GitHub-161B22?style=for-the-badge&logo=github&logoColor=FF9E64" alt="GitHub" />
</a>

<!--
  ┌──────────────────────────────────────────────────────────────────────────┐
  │  READY TO ENABLE — replace the URL, then delete the two comment markers. │
  │  Kept commented out so your live profile ships ZERO dead links.          │
  │  NOTE: simple-icons dropped the `linkedin` logo, so that badge is a      │
  │  text badge in LinkedIn blue rather than a logo badge.                   │
  └──────────────────────────────────────────────────────────────────────────┘

<a href="https://linkedin.com/in/YOUR-HANDLE">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&labelColor=0A66C2" alt="LinkedIn" />
</a>
<a href="https://YOUR-PORTFOLIO.com">
  <img src="https://img.shields.io/badge/Portfolio-161B22?style=for-the-badge&logo=vercel&logoColor=FF9E64" alt="Portfolio" />
</a>
<a href="https://x.com/YOUR-HANDLE">
  <img src="https://img.shields.io/badge/X-161B22?style=for-the-badge&logo=x&logoColor=C0CAF5" alt="X" />
</a>
<a href="https://buymeacoffee.com/YOUR-HANDLE">
  <img src="https://img.shields.io/badge/Buy_Me_a_Coffee-FFDD00?style=for-the-badge&logo=buymeacoffee&logoColor=1A1B26" alt="Buy Me a Coffee" />
</a>
-->

<br /><br />

<sub>💌 &nbsp;Best reached by email — I reply to everything that isn't a recruiter template.</sub>

</div>

<!-- ════════════════════════════════  QUOTE  ════════════════════════════════ -->

## 💬 &nbsp;引用 | Dev Quote

<div align="center">

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Random dev quote" />

</div>

<!-- ═══════════════════════════════  FOOTER  ════════════════════════════════ -->

<div align="center">

<br />

**⭐ If something here is useful to you, a star costs nothing and means a lot.**

<br />

<img
  src="https://capsule-render.vercel.app/api?type=waving&color=0:FF9E64,55:BB9AF7,100:1A1B26&height=150&section=footer&text=Thanks%20for%20stopping%20by&fontColor=C0CAF5&fontSize=22&fontAlignY=72"
  alt="Thanks for stopping by"
  width="100%"
/>

</div>
